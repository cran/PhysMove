...
The `plotDispPDF()` function requires a list of displacements calculated using `calcDisp()` and includes 3 optional parameters:
  ...
  * `normalise`: normalise the data before plotting, which divides all displacements in a time window by the mean displacement for that time window (`normalise=TRUE`, by default).
...
`plotDispPDF()` outputs a data frame of all data used to create the plot, including:
  ...
  * *pdf*: pdf values,
  * *disp*: binned displacement values (note that if `normalise=TRUE` the displacements are normalised values), and
...
```
**Figure V3** Probability density function (pdf) plot of normalised displacements from the `'tracks'` dataset calculated over 10 time windows, 24 to 240 hours at 24 ± 6-hour time intervals with `calcDisp()`. Plot created with `plotDispPDF()` default parameters (i.e., where `normalise=TRUE`). 
...
```
**Figure V4** Probability density function (pdf) plot of displacements from the `'tracks'` dataset calculated over 10 time windows, 24 to 240 hours at 24 ± 6-hour time intervals with `calcDisp()`. Plot created with `plotDispPDF()` where `normalise=FALSE`.
...
```
**Figure V6** Probability density function (pdf) plot of displacements calculated using `calcDisp()` with `max_hr=24`. Plot created with `plotDispPDF()` and `normalise=FALSE`.
